...
_ratodo() {
    ...
    commands="add done list status sync theme help -a -l"
    ...
    for (( i=1; i < COMP_CWORD; i++ )); do
        case "${COMP_WORDS[i]}" in
            add|-a) sub="add"; break ;;
            list|-l) sub="list"; break ;;
            done|status|sync|theme) sub="${COMP_WORDS[i]}"; break ;;
        esac
    done

    case "$sub" in
        list)   COMPREPLY=( $(compgen -W "--tag --prio --today --porcelain $global" -- "$cur") ) ;;
